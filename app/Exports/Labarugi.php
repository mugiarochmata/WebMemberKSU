...
class Labarugi implements FromView,WithStyles,ShouldAutoSize
{
    use Exportable;
    protected $pdate;
    ...
    {
        $data=Http::withToken(session('access_token'))->get(env('API_URL').'ledger/lr', ['pdate'=>$this->pdate])->json();;
        //dd($this->tdate);
        return view('laporan.labarugi-export', [
            'data' => $data,
            'pdate' => $this->pdate,
        ]);
    }
